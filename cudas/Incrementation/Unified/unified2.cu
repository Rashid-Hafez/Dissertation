...
//-----------------------------------------------------------
  ...
  while((bt*4)>mem){ //4 arrays in unified mem
    
    v.p += 2; //make 2 more partitions
    v1.p += 2; //make 2 more partitions
  ...
  //Malloc portion of array to be processed
  gpuErrchk(cudaMallocManaged(&v.vec, Nn*sizeof(unsigned long long)));
  gpuErrchk(cudaMallocManaged(&v1.vec, Nn*sizeof(unsigned long long)));
  
  gpuErrchk(cudaMallocManaged(&c, Nn*sizeof(unsigned long long)));
  gpuErrchk(cudaMallocManaged(&c1, Nn*sizeof(unsigned long long)));
  ...
  gpuErrchk(cudaStreamAttachMemAsync(stream1,v1.vec, 0,cudaMemAttachSingle));
  gpuErrchk(cudaStreamAttachMemAsync(stream1, c1, 0,cudaMemAttachSingle));

  gpuErrchk(cudaEventRecord(start,0));
  //----------------------START LOOP--------------------------------//

for (unsigned long long i = 0; i < v.M-v.overflow; i+=Nn*2){

    randomInit(v.vec, Nn);
    randomInit(v1.vec, Nn);

    Incr<<<GRID,BLOCK,0,stream0>>>(v.vec,c,Nn,i);
    Incr<<<GRID,BLOCK,0,stream1>>>(v1.vec,c1,Nn,i);
...
//----------------------END LOOP--------------------------------//
    ...
    gpuErrchk(cudaFree(v1.vec));
    gpuErrchk(cudaFree(c1));
